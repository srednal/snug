...
class TryIterableTest extends WordSpec with Matchers {
  ...
  "The TryIterable" should {
    ...
    "implicitly make a TryIterable" in {
      ...
      List(1, 2, 3).toTryIterable should {be(a[TryIterable[_]]) and be(a[Iterable[_]]) and contain(Success(1)) and contain(Success(2)) and contain(Success(3))}
    }
    "implicitly make a TryStream" in {
      import TryIterable._
      List(1, 2, 3).toTryStream should {be(a[Stream[_]]) and contain(Success(1)) and contain(Success(2)) and contain(Success(3))}
    }
  }
}
